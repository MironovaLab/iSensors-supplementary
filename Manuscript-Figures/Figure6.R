# Figure 6: iSensor activity across the Guo et al. 2025 multi-organ atlas
# Run with working directory = iSensors-supplementary/
#
# Input:  00-iSensors-objects/data/Guo/guo_avg_exp_iSensors_mean.rds
# Output: Manuscript-Figures/out/Figure6A_*.pdf / .svg
...
standardize_tissue <- function(x) {
  ...
  x <- if_else(x %in% c("bundle sheaths", "bundle-sheath"),                                       "bundle sheath", x)
  x
}
...
# Verify available sensor names
message("Available sensors after polish_name:")
print(sort(unique(all_data_agg$sensor)))


# ── Figure 6A: 4×1 stacked barplots — organs on X, all tissues colored ────────
# One panel per key iSensor; tissues as stacked segments within each organ.

sensors_6a <- c("ARF", "Synthesis","ConjugationDeconjugation", "PAT")


sensors_6a  <- intersect(sensors_6a, unique(all_data_agg$sensor))

# Consistent tissue order and colour palette across all 4 panels
tissue_order_6a <- all_data_agg %>%
  filter(sensor %in% sensors_6a) %>%
  distinct(organ, tissue_simple) %>%
  count(tissue_simple, name = "n_organs") %>%
  arrange(n_organs) %>%       # most common tissues anchor the bottom of each stack
  pull(tissue_simple)
...
make_bar_6a <- function(sensor_name, show_x = FALSE) {
  ...
  ggplot(df, aes(x = organ, y = value, fill = tissue_simple)) +
    ...
    scale_fill_manual(values = tissue_cols, name = "Cell type",
                      drop = FALSE) +
    scale_x_discrete(limits = organ_order[organ_order %in% levels(df$organ)]) +
    labs(x = NULL,
         y = c(ARF = "ARF", Synthesis = "Synthesis",
               ConjugationDeconjugation = "Conjugation", PAT = "PAT")[sensor_name]) +
    theme_classic(base_size = 10) +
    theme(
      axis.text.x     = if (show_x) element_text(angle = 45, hjust = 1, size = 8)
                        else element_blank(),
      axis.ticks.x    = if (show_x) element_line(linewidth = 0.3)
                        else element_blank(),
...
plots_6a <- lapply(sensors_6a, function(s) make_bar_6a(s, show_x = TRUE))
...
ggsave(file.path(output_dir, "Figure6A_stacked_barplot_4sensors.pdf"),
       plot = p_6a, width = 4 * length(sensors_6a), height = 6, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6A_stacked_barplot_4sensors.svg"),
       plot = p_6a, width = 4 * length(sensors_6a), height = 6, dpi = 300, bg = "white")
message("Saved Figure 6A (4×1)")

# ── Export underlying data ────────────────────────────────────────────────────
write.csv(
  filter(all_data_agg, sensor %in% sensors_6a),
  "Manuscript-Figures/in/Figure6A_avg_exp_per_organ_tissue.csv",
  row.names = FALSE
)
message("Exported Figure 6A data table")


# ══════════════════════════════════════════════════════════════════════════════
# Figures 6C–E: Linear model — ARF ~ Synthesis + Conjugation + PAT
# Input: Figure6A_avg_exp_per_organ_tissue.csv (generated above)
#        Long format: organ, tissue_simple, sensor, value
# ══════════════════════════════════════════════════════════════════════════════

fig6a_csv <- read.csv("Manuscript-Figures/in/Figure6A_avg_exp_per_organ_tissue.csv",
                      stringsAsFactors = FALSE)

df_wide <- fig6a_csv %>%
  filter(sensor %in% c("ARF", "Synthesis", "ConjugationDeconjugation", "PAT")) %>%
...
# Extract model stats for annotation
s      <- summary(m2)
...
# ── Figure 6C: Scatter — observed vs predicted ────────────────────────────────
arf_range <- range(df_wide$ARF, na.rm = TRUE)
band_6c <- tibble(x = seq(arf_range[1], arf_range[2], length.out = 200)) %>%
  mutate(lo = x - rmse, hi = x + rmse)

p_6c <- ggplot(df_wide, aes(x = ARF, y = ARF_hat)) +
  ...
  theme(axis.line  = element_line(linewidth = 0.4),
        ...
        panel.grid = element_blank())
p_6c
ggsave(file.path(output_dir, "Figure6C_ARF_scatter_observed_predicted.pdf"),
       plot = p_6c, width = 4, height = 4, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6C_ARF_scatter_observed_predicted.svg"),
       plot = p_6c, width = 4, height = 4, dpi = 300, bg = "white")
message("Saved Figure 6C")


# ── Figure 6D: Coefficient barplot ────────────────────────────────────────────
...
p_6d
ggsave(file.path(output_dir, "Figure6D_ARF_model_coefficients.pdf"),
       plot = p_6d, width = 3.5, height = 3, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6D_ARF_model_coefficients.svg"),
       plot = p_6d, width = 3.5, height = 3, dpi = 300, bg = "white")
message("Saved Figure 6D")


# ── Figure 6E: Forest plots — residuals at organ and tissue level ─────────────
...
p_6e <- p_6e_organ + p_6e_tissue

p_6e
ggsave(file.path(output_dir, "Figure6E_residuals_forest.pdf"),
       plot = p_6e, width = 10, height = 5, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6E_residuals_forest.svg"),
       plot = p_6e, width = 10, height = 5, dpi = 300, bg = "white")
message("Saved Figure 6E")


# ── Exploratory: forest plot of model residuals per reproductive cell type ─────
# Uses model coefficients from m2 (fitted on all tissues) applied to individual
# reproductive cell types (pollen, tapetum, integument, nectary, etc.) before
# they were merged by standardize_tissue().

repro_wide_raw <- all_data %>%
  filter(tissue_simple %in% c("male reproductive", "female reproductive"),
         sensor %in% c("ARF", "Synthesis", "ConjugationDeconjugation", "PAT")) %>%
...
repro_wide_raw$ARF_hat <- predict(m2, newdata = repro_wide_raw)
repro_wide_raw$resid   <- repro_wide_raw$ARF - repro_wide_raw$ARF_hat
...
p_repro_forest <- ggplot(repro_ct_summary,
  ...
  coord_flip() +
  labs(x = NULL,
       y = "Mean residual (observed ARF − predicted)") +
  theme_classic(base_size = 10) +
  theme(axis.line   = element_line(linewidth = 0.4),
        axis.ticks  = element_line(linewidth = 0.3),
...
ggsave(file.path(output_dir, "Figure6_reproductive_residuals_forest.pdf"),
       plot = p_repro_forest, width = 6, height = 5, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6_reproductive_residuals_forest.svg"),
       plot = p_repro_forest, width = 6, height = 5, dpi = 300, bg = "white")
message("Saved reproductive tissue residuals forest plot")


# ═══════════════════════════════════════════════════════════════════════════════
# Figure 6 combined: publication-ready layout
#
...
p_B <- ggplot() +
  ...
  annotate("text", x = 0.5, y = 0.38,
           label = expression("ARF = " * beta[1] * "·Synthesis + " *
...
# ── Panel E: organ + tissue forest side by side ───────────────────────────────
...
# patchwork leaf order (depth-first): 4 barplots in A, B, C, D, 2 in E, 2 in F
...
ggsave(file.path(output_dir, "Figure6_combined.pdf"),
       plot = fig6_combined,
       width = 20, height = 22, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6_combined.svg"),
       plot = fig6_combined,
       width = 20, height = 22, dpi = 300, bg = "white")
message("Saved Figure 6 combined: Figure6_combined.pdf / .svg")
