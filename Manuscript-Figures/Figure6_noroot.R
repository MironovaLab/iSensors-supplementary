# Figure 6 (no-root): iSensor activity across the Guo et al. 2025 multi-organ atlas
# Identical to Figure6.R but with Root 6 DAG and Root 11 DAG excluded.
...
ggsave(file.path(output_dir, "Figure6A_noroot_stacked_barplot_4sensors.pdf"),
       plot = p_6a, width = 4, height = 10, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6A_noroot_stacked_barplot_4sensors.svg"),
       plot = p_6a, width = 4, height = 10, dpi = 300, bg = "white")
message("Saved Figure 6A no-root (1×4)")

write.csv(
  filter(all_data_agg, sensor %in% sensors_6a),
  "Manuscript-Figures/in/Figure6A_noroot_avg_exp_per_organ_tissue.csv",
  row.names = FALSE
)
...
# ══════════════════════════════════════════════════════════════════════════════
# Figures 6C–E (no-root): Linear model — ARF ~ Synthesis + Conjugation + PAT
# ══════════════════════════════════════════════════════════════════════════════
...
p_6c <- ggplot(df_wide, aes(x = ARF, y = ARF_hat)) +
  ...
  theme(axis.line  = element_line(linewidth = 0.4),
        ...
        panel.grid = element_blank())
p_6c
ggsave(file.path(output_dir, "Figure6C_noroot_ARF_scatter.pdf"),
       plot = p_6c, width = 4, height = 4, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6C_noroot_ARF_scatter.svg"),
       plot = p_6c, width = 4, height = 4, dpi = 300, bg = "white")
...
p_6d <- ggplot(coef_df, aes(x = Term, y = Estimate)) +
  ...
  theme(axis.line  = element_line(linewidth = 0.4),
        ...
        panel.grid = element_blank())
p_6d
ggsave(file.path(output_dir, "Figure6D_noroot_ARF_model_coefficients.pdf"),
       plot = p_6d, width = 3.5, height = 3, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6D_noroot_ARF_model_coefficients.svg"),
       plot = p_6d, width = 3.5, height = 3, dpi = 300, bg = "white")
...
p_6e
ggsave(file.path(output_dir, "Figure6E_noroot_residuals_forest.pdf"),
       plot = p_6e, width = 4.5, height = 6, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6E_noroot_residuals_forest.svg"),
       plot = p_6e, width = 4.5, height = 6, dpi = 300, bg = "white")
...
ggsave(file.path(output_dir, "Figure6_noroot_reproductive_residuals_forest.pdf"),
       plot = p_repro_forest, width = 4.5, height = 1.5, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6_noroot_reproductive_residuals_forest.svg"),
       plot = p_repro_forest, width = 4.5, height = 1.5, dpi = 300, bg = "white")
...
ggsave(file.path(output_dir, "Figure6_noroot_combined.pdf"),
       plot = fig6_combined,
       width = 20, height = 22, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6_noroot_combined.svg"),
       plot = fig6_combined,
       width = 20, height = 22, dpi = 300, bg = "white")
message("Saved Figure 6 no-root combined: Figure6_noroot_combined.pdf / .svg")
