...
library(ggpubr)
library(circlize)
try(library(ComplexHeatmap), silent=TRUE) # stubbed
library(tidyverse)
library(ggrepel)
library(ggPlantmap)
...
iSensors_obj <-readRDS(file = "00-iSensors-objects/data/shahan-iSensors-obj-groundtruth.rds")
...
if (!requireNamespace("ggPlantmap", quietly = TRUE)) {
  invisible;# devtools::install_github("leonardojo/ggPlantmap")
} else {
...
if (!requireNamespace("ggRootCellAtlas", quietly = TRUE)) {
  invisible;# devtools::install_github("MariaSavina91/ggRootCellAtlas")
} else {
...
iSensors_obj <- readRDS("00-iSensors-objects/data/shahan-roottip-iSensors-obj.rds")
Assays(iSensors_obj)
DefaultAssay(iSensors_obj)<- "iSensors_mean"
avg_exp  <- AverageExpression(iSensors_obj, assays = "iSensors_mean", slot = "data",
                              verbose = FALSE)
...
in_dir_stats <- "Manuscript-Figures/in"
