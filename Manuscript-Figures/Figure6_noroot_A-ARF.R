# Figure 6 (no-root, A-ARF variant): iSensor activity across the Guo et al.
# 2025 multi-organ atlas. Identical to Figure6_noroot.R but the linear model
# uses the A-ARF trans-panel (the 5 classic activator ARFs: ARF5/MP, ARF6,
# ARF7/NPH4, ARF8, ARF19 - a functional subset of the 22-gene ARF panel) as
# the response variable instead of the full ARF panel:
#   A-ARF = beta1*Synthesis + beta2*Conjugation + beta3*PAT
# All outputs are saved under distinct "A-ARF" filenames so the original
# ARF-based figure/CSV outputs are not overwritten.
#
# Implementation note: "A-ARF" is not a syntactically valid R name (the
# hyphen would be parsed as subtraction in formulas/`$` access), so the
# response column is renamed to a generic "Response" immediately after
