# Figure 6 (no-root, IAA variant): iSensor activity across the Guo et al. 2025
# multi-organ atlas. Identical to Figure6_noroot.R but the linear model uses
# the IAA trans-panel (Aux/IAA primary-response genes) as the response
# variable instead of ARF (Auxin Response Factor TFs):
#   IAA = beta1*Synthesis + beta2*Conjugation + beta3*PAT
# All outputs are saved under distinct "_IAA" filenames so the original
# ARF-based figure/CSV outputs are not overwritten.
...
sensors_6a <- c("IAA", "Synthesis", "ConjugationDeconjugation", "PAT")
...
ggsave(file.path(output_dir, "Figure6A_noroot_IAA_stacked_barplot_4sensors.pdf"),
       plot = p_6a, width = 4, height = 10, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6A_noroot_IAA_stacked_barplot_4sensors.svg"),
       plot = p_6a, width = 4, height = 10, dpi = 300, bg = "white")
message("Saved Figure 6A no-root (1×4)")

write.csv(
  filter(all_data_agg, sensor %in% sensors_6a),
  "Manuscript-Figures/in/Figure6A_noroot_IAA_avg_exp_per_organ_tissue.csv",
  row.names = FALSE
)
...
# ══════════════════════════════════════════════════════════════════════════════
# Figures 6C–E (no-root): Linear model — IAA ~ Synthesis + Conjugation + PAT
# ══════════════════════════════════════════════════════════════════════════════
...
df_wide <- fig6a_csv %>%
  filter(sensor %in% c("IAA", "Synthesis", "ConjugationDeconjugation", "PAT")) %>%
...
m2 <- lm(IAA ~ 0 + Synthesis + Conjugation + PAT, data = df_wide)

df_wide$IAA_hat <- predict(m2)
...
# ── Figure 6C (no-root) ───────────────────────────────────────────────────────
iaa_range <- range(df_wide$IAA, na.rm = TRUE)
band_6c <- tibble(x = seq(iaa_range[1], iaa_range[2], length.out = 200)) %>%
  mutate(lo = x - rmse, hi = x + rmse)

p_6c <- ggplot(df_wide, aes(x = IAA, y = IAA_hat)) +
  ...
  coord_equal(xlim = iaa_range, ylim = iaa_range) +
  annotate("text", x = iaa_range[1], y = iaa_range[2],
           label = paste0("adj. R² = ", r2_adj, "\n", plab),
           hjust = 0, vjust = 1, size = 3.2, color = "grey30") +
  labs(x = "Observed IAA iSensor", y = "Predicted auxin level") +
  theme_classic(base_size = 11) +
  theme(axis.line  = element_line(linewidth = 0.4),
        axis.ticks = element_line(linewidth = 0.3),
        panel.grid = element_blank())
p_6c
ggsave(file.path(output_dir, "Figure6C_noroot_IAA_scatter.pdf"),
       plot = p_6c, width = 4, height = 4, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6C_noroot_IAA_scatter.svg"),
       plot = p_6c, width = 4, height = 4, dpi = 300, bg = "white")
...
p_6d <- ggplot(coef_df, aes(x = Term, y = Estimate)) +
  ...
  theme(axis.line  = element_line(linewidth = 0.4),
        ...
        panel.grid = element_blank())
p_6d
ggsave(file.path(output_dir, "Figure6D_noroot_IAA_model_coefficients.pdf"),
       plot = p_6d, width = 3.5, height = 3, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6D_noroot_IAA_model_coefficients.svg"),
       plot = p_6d, width = 3.5, height = 3, dpi = 300, bg = "white")
...
p_6e
ggsave(file.path(output_dir, "Figure6E_noroot_IAA_residuals_forest.pdf"),
       plot = p_6e, width = 4.5, height = 6, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6E_noroot_IAA_residuals_forest.svg"),
       plot = p_6e, width = 4.5, height = 6, dpi = 300, bg = "white")
...
repro_wide_raw <- all_data %>%
  filter(tissue_simple %in% c("male reproductive", "female reproductive"),
         sensor %in% c("IAA", "Synthesis", "ConjugationDeconjugation", "PAT")) %>%
  ...
  drop_na(IAA, Synthesis, Conjugation, PAT) %>%
  mutate(cell_type = str_to_title(
    str_replace_all(str_remove(tissue, "-\\d+$"), "[_-]", " ")
  ))

repro_wide_raw$IAA_hat <- predict(m2, newdata = repro_wide_raw)
repro_wide_raw$resid   <- repro_wide_raw$IAA - repro_wide_raw$IAA_hat
...
p_repro_forest <- ggplot(repro_ct_summary,
  ...
  coord_flip() +
  labs(x = NULL,
       y = "Mean residual (observed IAA − predicted)") +
  theme_classic(base_size = 10) +
  theme(axis.line   = element_line(linewidth = 0.4),
        axis.ticks  = element_line(linewidth = 0.3),
...
ggsave(file.path(output_dir, "Figure6_noroot_IAA_reproductive_residuals_forest.pdf"),
       plot = p_repro_forest, width = 4.5, height = 1.5, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6_noroot_IAA_reproductive_residuals_forest.svg"),
       plot = p_repro_forest, width = 4.5, height = 1.5, dpi = 300, bg = "white")
...
p_B <- ggplot() +
  ...
  annotate("text", x = 0.5, y = 0.38,
           label = expression("IAA = " * beta[1] * "·Synthesis + " *
...
ggsave(file.path(output_dir, "Figure6_noroot_IAA_combined.pdf"),
       plot = fig6_combined,
       width = 20, height = 22, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "Figure6_noroot_IAA_combined.svg"),
       plot = fig6_combined,
       width = 20, height = 22, dpi = 300, bg = "white")
message("Saved Figure 6 no-root combined: Figure6_noroot_IAA_combined.pdf / .svg")
